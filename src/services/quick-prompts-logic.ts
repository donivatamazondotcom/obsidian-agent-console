...
export const NEW_PROMPT_BODY_PLACEHOLDER = [
	...
	"- `show when`: the chip shows only on matching notes. Add one list item per condition, like `type=meeting`, `tags=people`, or `status=open`. A bare property name like `source-url`, for example, matches any note that has it, whatever its value.",
	"- `show when any`: leave it unchecked and every condition listed in `show when` must match. Check it and the chip shows when any one of them matches.",
