...
By default the agent is told five things:
...
- **It can offer clickable choices** — when the agent has a small set of options for you, it can show them as buttons in its reply. See [Clickable choices](/usage/interactive-buttons).
...
Each of the items above has its own switch — turn off anything you'd rather not send. The agent still works either way; these just help it fit in better.
