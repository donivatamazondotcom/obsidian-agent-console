...
features:
  ...
  - icon: 🤖
    ...
    details: Claude Code, Codex, Gemini CLI, Kiro CLI, or any custom ACP-compatible agent. Run several side by side in a tabbed sidebar – each tab its own session, surviving an Obsidian restart (or just closing and reopening the panel) and connecting to its agent only when you start typing. On a fresh install it defaults to an agent you already have installed, so your first chat just works. Close the panel with multiple chats open and it checks before dropping them all.
  - icon: 💻
