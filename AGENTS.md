...
│   ├── composer-affordances.ts  # deriveComposerAffordances — pure composer send-target + control-composition resolver (surface × lazyState × composer-caps × hasQuickPrompts → sendMode/quickPromptFire/context/showAttachments/showConfigSelectors); shared by the landing + in-tab composer so they can't drift; layers with deriveSendAffordance (enablement) — no overlap
